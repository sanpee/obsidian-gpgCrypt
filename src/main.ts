import { Notice, Platform, Plugin, DataWriteOptions, TFile, normalizePath, setIcon, App, TAbstractFile, TFolder, FileView } from "obsidian";
...
import GpgImageView from "./image/GpgImageView";
import GpgImagePostProcessor from "./image/MarkupPostProcessor";
import GpgImage from "./image/GpgImage";
...
export default class GpgPlugin extends Plugin {
  ...
  private postProcessor!: GpgImagePostProcessor;
  ...
  async onload() {
    ...
    this.registerView(GpgImageView.VIEW_TYPE, (leaf) => new GpgImageView(leaf, async file => {
      const buf = await this.app.vault.readBinary(file);
      return this.decryptImage(buf);
    }));

    // Register the file extension and associate with the view
    this.registerExtensions(GpgImage.GpgImageExtension, GpgImageView.VIEW_TYPE);

    this.postProcessor = new GpgImagePostProcessor(this, async file => {
      const buf = await this.app.vault.readBinary(file);
      return this.decryptImage(buf);
    }
    );

    // TODO: Need to find out if need to control which part of chain this should be in, end or start
    this.registerMarkdownPostProcessor(this.postProcessor.process.bind(this.postProcessor));
  }
  ...
  override async onunload(): Promise<void> {
    ...
    this.app.workspace.getLeavesOfType(GpgImageView.VIEW_TYPE).forEach(l => l.detach());
  ...
  // Assuming this plugin only encrypt to armored text, binary files are for images
  ...
  private async hookedAdapterReadBinary(normalizedPath: string): Promise<ArrayBuffer> {
    ...
    const content = await this.originalReadBinary(normalizedPath);
    const isEncrypted = GpgImage.isEncrypted(content);
  ...
  // TODO: Need to support CLI wrapper
  async encryptBinary(data: ArrayBuffer): Promise<ArrayBuffer> {
    if (this.settings.backend == Backend.NATIVE) {
      ...
      return this.gpgNative.encrypt(data);
    } else {
      throw new Error("TODO: Support native for now!")
    }
  }
  ...
  async decryptImage(data: ArrayBuffer): Promise<ArrayBuffer> {
    if (this.settings.backend == Backend.NATIVE) {
      ...
      while (true) {
        try {
          ...
          const decrypted = await this.gpgNative.decrypt(data, passphrase);
      ...
      // TODO: Need to support CLI wrapper
      new Notice("CLI Wrapper not yet support!");
      throw new Error("CLI Wrapper not yet support!");
    }
  }
  ...
  async persistentFileEncrypt(file: TFile) {
    if (file.extension === "md") {
      this.encryptMarkdownFile(file);
    }
    else if (GpgImage.hasNativeImageExtension(file.path)) {
      _log(`Encrytping image file: ${file.path}`)
      this.encryptImageFile(file);
    }
  }

  async encryptImageFile(file: TFile) {
    try {
      ...
      if (!this.encryptedFileStatus.has(file.path)) {
        ...
        throw Error("Image is already encrypted!");
      }

      // if (this.settings.renameToGpg === true && GpgImage.NativeImageExtension.some(x=>x===file.extension)) {
      if (GpgImage.NativeImageExtension.some(x => x === file.extension)) {
        _log(`rename to GpgImage: ${file.path}`)
        const newPath = GpgImage.changeFileExt(file.path);
        try {
          // TODO: Should allow user to choose in place encryption or generate a new file
          // Create a new empty file to store the encrypted data
          // file = await this.app.vault.createBinary(normalizePath(newPath), new ArrayBuffer, {ctime: file.stat.ctime, mtime: file.stat.mtime})
          ...
          throw Error(`Encryption failed as image could not be renamed to GpgImage file extension: ${error}`);
        }
      }

      const contentEncrypted = await this.encryptBinary(content);
      await this.app.vault.adapter.writeBinary(file.path, contentEncrypted);
  ...
  async encryptMarkdownFile(file: TFile) {
    try {
  ...
  // Switch decryption based on file extension
  async persistentFileDecrypt(file: TFile) {
    if (file.extension === "md") {
      this.encryptMarkdownFile(file);
    }
    else if (GpgImage.hasSupportedExtension(file.path)) {
      _log(`Decrytping image file: ${file.path}`)
      this.decryptImageFile(file);
    }
  }

  // Decrypt the file in-place persistently when the user manually triggers the decryption
  async decryptMarkdownFile(file: TFile) {
    try {
      // this should at the end call our hookedAdapterRead
      // and will set encryptedFileStatus
      const content = await this.app.vault.read(file);
  ...
  async decryptImageFile(file: TFile) {
    try {
      // this should at the end call our hookedAdapterRead
      // and will set encryptedFileStatus
      const content = await this.app.vault.readBinary(file);

      // Since the content is read, will check if this is an encrypted file
      /*
      if (GpgImage.isEncrypted(content)) {
        this.encryptedFileStatus.set(file.path, true);
      }
      */
      ...
      if (!this.encryptedFileStatus.has(file.path)) {
        ...
        throw Error(`${file.path} is not encrypted!`);
      }

      const orgPath = file.path;
      if (GpgImage.hasSupportedExtension(file.path)) {
        const newPath = GpgImage.removeFileExt(file.path);
        try {
          await this.app.fileManager.renameFile(file, newPath);
          // refresh file as the file extension changed
          file = this.app.vault.getAbstractFileByPath(normalizePath(newPath)) as TFile;
        } catch (error) {
          throw Error(`Decryption failed as binary file could not be renamed to image file extension: ${error}`);
        }
      }

      const contentDecrypted = await this.decryptImage(content);
      await this.app.vault.adapter.writeBinary(file.path, contentDecrypted);
      await this.app.workspace.openLinkText("", file.path, false);
      this.encryptedFileStatus.delete(orgPath);
      this.statusBarRefresh(file);
